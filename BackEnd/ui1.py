...
class TrainWorker(QThread):
    ...
    def run(self):
        # Import trainAgent dynamically
        sys.path.append(os.path.dirname(__file__))  # ensure BackEnd folder is in path
        try:
        ...
        try:
            trainAgent()  # call the function directly
        except Exception as e:
            self.log_signal.emit(f"Error during training: {e}")
        finally:
            sys.stdout = old_stdout

        # Emit captured output line by line
...
class MainApp(QMainWindow):
    def __init__(self):
        ...
        # Splitter for reward + winrate plots
    ...
    def update_battle_plot(self, win_rates):
        ...
        ax.set_title("Win Rate vs Human")
